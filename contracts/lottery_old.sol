...
interface BQBToken {
    ...
    function transferPrizeReserve(address _toLottery, uint256 _amount) external;
}
...
contract BloqBallLottery is ReentrancyGuard, IBloqBallSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInBQB = 200 * 10 ** 18;        // 200 BQB
    uint256 public minPriceTicketInBQB = 1 * 10 ** 18;          // 1 BQB

    uint256 public pendingInjectionNextLottery;
    uint256 public prizeReserve = 500000 * 10 ** 18;            // 500BQB
    ...
    uint256 private minPendingInjectionNextLottery = 20000 * 10 ** 18;  // 20K BQB
    uint256 private minPrizeReserve = 10000 * 10 ** 18;                 // 10K BQB
    uint256 public priceTicketInBQB;
   ...
   // Keep track of user wining rewards for a given lotteryId
    ...
    /**
     * @notice Generate tickets numbers for the current buyer
     * @dev Callable by users
     */
    function generatebulkTicketNumbers(uint256[] memory _ticketNumbers) private view {
...
//              _itemNumber = (randomGenerator.viewRandomResult() % 21) + 1;
                _itemNumber = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, i))) % 21 + 1;
...
//          _itemNumber = (randomGenerator.viewRandomResult() % 21) + 1;
    ...
    // get the total players of the lotteris.
        ...
        returns (uint256[] memory new_ticketIds, uint256[] memory new_brackets, uint256 pendingRewards)
    {
        ...
        for (uint i = 0; i < ticketLength; i++)
        {
            ...
            uint j;
            for (j = 0; j < 6; j++) {
                number1 = ticketNumbers[i] / 10 ** (j*2);
                number2 = _winningNumber / 10 ** (j*2);
                
                // If the winning numbers and user numbers match
                if(number1 == number2) {
                    _brackets[i] = uint32(5 - j);
                    index ++;
        ...
        for (uint i = 0; i < ticketLength; i++)
        {
            ...
            pendingRewards += _calculateRewardsForTicketId(_lotteryId, _ticketIds[i], _brackets[i]);
        }
    }
    ...
    function claimTickets(
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardForTicketId = _calculateRewardsForTicketId(_lotteryId, thisTicketId, _brackets[i]);
    ...
    function checkLotteryState() external onlyOperator{
        ...
        else if (block.timestamp >= _lotteries[currentLotteryId].endTime + 5 * 60 
        && _lotteries[currentLotteryId].status == Status.Claimable)
        {
            ...
            if (!weeklyLottery)
            {
                _endTime = block.timestamp + 24 hours - 60;     // 23:59:59
                _priceTicketInBQB = 100 * 10 ** 18;
            }
            else
            {
                _endTime = block.timestamp + 7 days - 60;       // 6 days and 23:59:59
                _priceTicketInBQB = 200 * 10 ** 18;
            }
            
            startLottery(_endTime, _priceTicketInBQB, _lotteries[currentLotteryId].discountDivisor, 
        ...
        nonReentrant
    {
        ...
        for (uint32 i = 0; i < 6; i++) {
            uint32 j = 5 - i;
            uint256 transformedWinningNumber = _bracketCalculator[j] + (finalNumber / (uint256(10)**((5 - j)*2)));
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].BQBPerBracket[j] =
                ...
                _lotteries[_lotteryId].BQBPerBracket[j] = 0;
                ...
                pendingInjectionNextLottery += (_lotteries[_lotteryId].rewardsBreakdown[j] * amountToShareToWinners) * 7500 /
                    (10000 * 10000);
                prizeReserve += (_lotteries[_lotteryId].rewardsBreakdown[j] * amountToShareToWinners) * 2500 /
                    (10000 * 10000);
            }
        }
    ...
    function startLottery(
        ...
        // daily lottery : minPendingInjectionNextLottery = 20000 * 10 ** 18 (20K BQB), minPrizeReserve = 10000  * 10 ** 18 (10K BQB)
        // Weekly bumper lottery : minPendingInjectionNextLottery = 40000 * 10 ** 18 (40K BQB), minPrizeReserve = 20000  * 10 ** 18 (20KBQB)
        if (!weeklyLottery)
        {
            minPendingInjectionNextLottery = 20000 * 10 ** 18;
            minPrizeReserve = 10000 * 10 ** 18;      
        }
        else{
            minPendingInjectionNextLottery = 40000 * 10 ** 18;
            minPrizeReserve = 20000 * 10 ** 18;      
        }
     ...
     * @param _minPriceReserve: maximum value of a prize reserve
     */
    function setMinPendingInjectionNextLotteryAndPrizeReserve(uint256 _minPendingInjectionNextLottery, uint256 _minPriceReserve)
        external
        onlyOperator
    {
        minPendingInjectionNextLottery = _minPendingInjectionNextLottery;
        minPrizeReserve = _minPriceReserve;
    }
     ...
     * @dev Only callable by owner
     */
    function setMaxNumberTicketsPerBuy(uint256 _maxNumberTicketsPerBuy) external onlyOperator {
     ...
     */
    function setWeeklyLottery(bool enable) external onlyOwner {
    ...
    function _calculateRewardsForTicketId(
        ...
        uint256 transformedWinningNumber = _bracketCalculator[_bracket] + (winningTicketNumber  / (uint256(10)**((5 - _bracket)*2)));

        uint256 transformedUserNumber = _bracketCalculator[_bracket] + (userNumber / (uint256(10)**((5 - _bracket)*2)));
